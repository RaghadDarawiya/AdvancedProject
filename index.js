...
app.post("/sign/employer", (req,res)=>{
  ...
  db.query('SELECT * FROM employers WHERE email = ?', [email], (err, results) => {
    ...
    db.query( 'INSERT INTO employers (id, companyname, email, phone , password) VALUES (?, ?, ?, ? ,?)',
    [this.id, this.companyname, this.email , this.phone , this.password], (err, result) => {
      if (err) {
...
app.post("/sign/seeker", (req,res)=>{
  const {seeker_id , fname, lname, email, resume} = req.body;
  ...
  // Create a new instance of the JobListing model
    ...
    // Insert the new job seeker into the database
    db.query('INSERT INTO jobSeekers (seeker_id , fname, lname, email, resume) VALUES (? , ?, ?, ?, ?)', [seeker_id , fname, lname, email, resume], (err, result) => {
      if (err) {
        console.error('Error signing up:', err);
        res.status(500).json({ error: 'An error occurred while signing up' });
      } else {
